...
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_params, normalization):
    ...
    if normalization == 'batchnorm':
        ...
        bn_out, bn_cache = layernorm_forward(a, gamma, beta, bn_params)
...
def affine_bn_relu_backward(dout, cache, normalization):
    ...
    da = relu_backward(dout, relu_cache)
    if normalization == 'batchnorm':
        dbn_out, dgamma, dbeta = batchnorm_backward_alt(da, bn_cache)
    elif normalization == 'layernorm':
        dbn_out, dgamma, dbeta = layernorm_backward(da, bn_cache)
...
def conv_relu_backward(dout, cache):
    ...
    da = relu_backward(dout, relu_cache)
    dx, dw, db = conv_backward_naive(da, conv_cache[:-1])
    return dx, dw, db
